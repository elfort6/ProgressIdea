...
if($fila['total']!=0){//si el usuario o el numID ya está registrado
    ...
    $insertPersona=mysqli_query($conexion, 'INSERT INTO persona(idRegistro, primerNombrel, segundoNombre, primerApellido, segundoApellido, Correo_idCorreo, Telefono_idTelefono, numId, direccion, codigoPostal, fechaNacimiento, pais) VALUES ("'.$idRandom.'", "'.$_POST['nombre'].'", "'.$_POST['snombre'].'", "'.$_POST['apellido'].'", "'.$_POST['sapellido'].'", "'.$idRandom.'", "'.$idRandom.'", "'.$_POST['identidad'].'", "'.$_POST['direccion'].'", "'.$_POST['codPostal'].'", "'.$_POST["fechaNacimiento"].'","'.$nombrePais.'")')
    or die('<p>Error al registrar</p><br>'.mysqli_error($conexion));
